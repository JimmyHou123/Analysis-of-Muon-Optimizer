...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        # 1) compute raw attention scores and weights manually
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)
        if attn_mask is not None:
            scores = scores + attn_mask
        attn_weights = torch.softmax(scores, dim=-1)
        # 2) stash for your basic‐stats pipeline
        self.last_attn = attn_weights.detach().cpu()    # shape: (bs, heads, seq, seq)
        # 3) then use the fused kernel for the output projection
