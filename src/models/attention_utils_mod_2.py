...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, qk_norm=False):
        ...
        # storage for last-forward attention weights
        self.last_attn = None
    ...
    def forward(
        ...
        # 2) split heads → (bs, n_heads, seq_len, head_dim)
        q = q.view(bs, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        k = k.view(bs, k_len, self.num_heads, self.head_dim).transpose(1, 2)
        v = v.view(bs, k_len, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        # 4) rotary embeddings & cached key/value
        ...
        # 5) merge padding & attention masks
        ...
        # 6) set dropout probability
        dropout_p = 0.0 if not self.training else self.dropout
    
        # 7) manual scaled-dot-product attention
        #    raw scores: (bs, n_heads, seq_len, k_len)
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)
        if attn_mask is not None:
            # additive mask if float, or bool mask otherwise
            try:
