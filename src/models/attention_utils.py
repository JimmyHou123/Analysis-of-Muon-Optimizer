...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        # 1) Raw dot-product scores (unnormalized):
        #    shape = (batch, heads, seq_len, seq_len)
        # scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)
        # A) Capturing raw V for later inspection
        #    shape = (bs, n_heads, seq_len, head_dim)
        raw_v = v.detach().cpu()
        self.last_raw_v = raw_v

        # 2) Apply masks:
        # if attn_mask is not None:
        #     # attn_mask is additive: assume it contains -inf for masked positions
        #     scores = scores + attn_mask
        # key_padding_mask should already have been folded into attn_mask above

        # # 3) Compute attention weights
        # weights = torch.softmax(scores, dim=-1)
        # ————————————————————————————————
        # B) (unchanged) Compute attention weights from Q/K
        scores  = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)
        weights = torch.softmax(scores, dim=-1)

        # 4) (Optional) Dropout on weights
        if dropout_p > 0.0:
            weights = F.dropout(weights, p=dropout_p, training=self.training)

        # # 5) Save for later inspection
        # self.last_scores = scores.detach().cpu()    # raw scores
        # self.last_attn   = weights.detach().cpu()   # normalized weights
        # (you can still keep these if you want)
        ...
        # merge heads
        output_flat = output.transpose(1, 2).contiguous().view(bs, seq_len, -1)

        # D) Apply the final linear out_proj
        proj_out = self.out_proj(output_flat)

        # E) stash the post-projection activations
        #    shape = (bs, seq_len, embed_dim)
        self.last_out_proj = proj_out.detach().cpu()
