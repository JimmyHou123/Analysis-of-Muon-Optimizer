###############
# muon only optimizes attention layer
###############
...
from models.attention_utils import MultiheadAttention, MultiheadFlexAttention
...
class Trainer(object):
    def __init__(self, modules, params, symbol_env):
        ...
        # will accumulate lists of per-layer tensors
        self.attn_buffer = []  
    ...
    def set_optimizer(self):
        ...
        match params.optim.type:
            ...
            case "muon":
                ...
                # parameters containing these substrings will be sent to AdamW
                adam_keys = ["embed"]
                ...
                for n, p in named_params:
                    if p.requires_grad:
                        # Muon only on 2D attention weights (i.e. anything under .self_attn.)
                        if p.ndim >= 2 and ".self_attn." in n and not any(key in n for key in adam_keys):
            ...
            case "shampoo":
                self.optimizer = Shampoo(
                    self.parameters["model"],                          # parameter list
                    lr=params.optim.lr,                                # from shampoo.yaml
                    momentum=params.optim.get("momentum", 0.0),        # optional
                    weight_decay=params.optim.weight_decay,            # L2 penalty
                    epsilon=params.optim.get("eps", 1e-8),             # numerical safeguard
                    block_size=params.optim.get("block_size", 128),
                )
    ...
    def end_epoch(self):
        if len(self.attn_buffer) > 0:
            self.compute_and_log_attn_stats()
            self.compute_layer_head_stats()
    ...
    def iter(self):
        ...
        # clear attn_buffer at the start of each epoch
        if self.inner_epoch % self.n_steps_per_epoch == 0:
            self.attn_buffer.clear()
        ...
        with torch.amp.autocast("cpu" if params.cpu else "cuda",
                                enabled=bool(params.amp),
                                dtype=torch.float16):
            # 1) forward pass
            data_output = model("fwd", **model_input)

            # 2) collect attention weights from every layer
            layers = (model.module.transformer.layers
                      if hasattr(model, "module")
                      else model.transformer.layers)
            all_attn = [
                layer.self_attn.last_attn.detach().cpu()
                for layer in layers
            ]
            self.attn_buffer.append(all_attn)
    ...
    def compute_and_log_attn_stats(self):
        # concatenate all layers & batches into one flat tensor
        flat = torch.cat([
            attn.reshape(-1)
            for batch in self.attn_buffer
            for attn in batch
        ], dim=0)
        ...
        std   = flat.std().item()
        p5, p25, p75, p95 = torch.quantile(
            flat, torch.tensor([0.05, 0.25, 0.75, 0.95])
        ).tolist()

        logger.info(
            f"[Attn Stats] mean={mean:.4f}, med={med:.4f}, std={std:.4f}, "
            f"5%={p5:.4f},25%={p25:.4f},75%={p75:.4f},95%={p95:.4f}"
        )

    def compute_layer_head_stats(self):
        # Number of transformer layers
        n_layers = len(self.attn_buffer[0])
    
        # Concatenate across all batches for each layer
        layer_tensors = [
            torch.cat([batch_attn[layer_idx] for batch_attn in self.attn_buffer], dim=0)
            for layer_idx in range(n_layers)
        ]
    
        # Determine number of heads from first layer tensor: (B_total, H, L, L)
        _, H, L, _ = layer_tensors[0].shape
    
        # Allocate arrays for per‐layer, per‐head statistics
        mean_per_head   = np.zeros((n_layers, H))
        median_per_head = np.zeros((n_layers, H))
    
        # Compute mean & median for each (layer, head)
        for l, attn in enumerate(layer_tensors):
            for h in range(H):
                vals = attn[:, h].reshape(-1).cpu().numpy()
                mean_per_head[l, h]   = vals.mean()
                median_per_head[l, h] = np.median(vals)
    
        # Plot heatmap of mean attention per head vs. layer
        plt.figure()
        plt.imshow(mean_per_head, aspect='auto')
        plt.xlabel("Head")
        plt.ylabel("Layer")
        plt.title("Mean Attention per Head vs Layer")
